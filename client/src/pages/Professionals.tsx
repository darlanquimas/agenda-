...
function AvailabilityEditor({ value, onChange }: { value: AvailSlot[]; onChange: (v: AvailSlot[]) => void }) {
  ...
  const removePeriod = (wd: number, pIdx: number) => {
    let count = -1;
      ...
      {grouped.map(({ wd, periods }) => (
        <div key={wd} className="rounded-lg border border-gray-800 overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 bg-gray-800/60">
            ...
            <button type="button" onClick={() => addPeriod(wd)} className="flex items-center gap-1 text-xs text-indigo-400 hover:text-indigo-300 transition-colors"><Plus size={12} /> Faixa</button>
          </div>
          <div className="divide-y divide-gray-800/60">
            {periods.map((period, pIdx) => (
              <div key={pIdx} className="flex items-center gap-2 px-3 py-2.5">
                <Clock size={13} className="text-gray-600 shrink-0" />
                <input type="time" className="input py-1.5 text-xs w-28" value={period.start_time} onChange={(e) => updatePeriod(wd, pIdx, 'start_time', e.target.value)} />
                <span className="text-gray-600 text-xs shrink-0">às</span>
                <input type="time" className="input py-1.5 text-xs w-28" value={period.end_time} onChange={(e) => updatePeriod(wd, pIdx, 'end_time', e.target.value)} />
                {periods.length > 1 && (
                  <button type="button" onClick={() => removePeriod(wd, pIdx)} className="ml-auto p-1 text-gray-600 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors"><Minus size={14} /></button>
                )}
              </div>
            ))}
          </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {allSpecialties.map((s) => { const active = form.specialty_ids.includes(s.id); return (
            <button key={s.id} type="button" onClick={() => toggleId('specialty_ids', s.id)}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {allServices.map((s) => { const active = form.service_ids.includes(s.id); return (
            <button key={s.id} type="button" onClick={() => toggleId('service_ids', s.id)}
